...
use App\Service\ContentAnalyzer; // Ajoutez cette ligne
...
class BlogArticleController extends AbstractController
{
    ...
    private ContentAnalyzer $contentAnalyzer; // Ajoutez cette propriété

    public function __construct(EntityManagerInterface $entityManager, ContentAnalyzer $contentAnalyzer) // Modifiez le constructeur
    {
        $this->entityManager = $entityManager;
        $this->contentAnalyzer = $contentAnalyzer; // Initialisez la propriété
    }
    ...
    public function create(Request $request): JsonResponse
    {
        ...
        // Content validation using ContentAnalyzer
        $bannedKeywords = ['bannedWord1', 'bannedWord2']; // Exemple de mots interdits
        if (!$this->contentAnalyzer->validateContent($data['content'], $bannedKeywords)) {
            return $this->json(['error' => 'Content contains banned words.'], JsonResponse::HTTP_BAD_REQUEST);
        }

        // Proceed to create the blog article
        $user = (object) ['id' => 1]; // Author ID
    ...
    public function update(Request $request, int $id): JsonResponse
    {
        ...
        if (isset($data['content']) && !empty($data['content'])) {
            // Validate content again on update
            $bannedKeywords = ['bannedWord1', 'bannedWord2']; // Exemple de mots interdits
            if (!$this->contentAnalyzer->validateContent($data['content'], $bannedKeywords)) {
                return $this->json(['error' => 'Content contains banned words.'], JsonResponse::HTTP_BAD_REQUEST);
            }
